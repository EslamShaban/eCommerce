...
<div class="card">
    <div class="card-header">
      <h3 class="card-title">{{trans('admin.shippings')}}</h3>
    </div>
    <!-- /.card-header -->
    <div class="card-body">
      <a href="{{route('shippings.create')}}" class="btn btn-info"><i class="fa fa-plus"></i> <span>{{trans('admin.create_new_shipping')}}</span></a>
      ...
      <form action="{{aurl('shippings/destroy/all')}}" id="form_data" >
        ...
        <table id="example1" class="table table-bordered table-striped">
          ...
          <tr>
            ...
            <th>#</th>
            <th>{{trans('admin.shippingname_ar')}}</th>
            <th>{{trans('admin.shippingname_en')}}</th>
          ...
          <tbody>
              ...
              @foreach($shippings as $shipping)
                  ...
                  <tr style="height:10px">
                    <td><input type="checkbox" class="item_checkbox" name="item[]" onclick="item_check('{{$shippings->count()}}')" value="{{$shipping->id}}" /></td>
                    <td>{{$i}}</td>
                    <td>{{$shipping->shippingname_ar}}</td>
                    <td>{{$shipping->shippingname_en}}</td>
                    <td>{{$shipping->user->name}}</td>
                    <td>{{$shipping->created_at}}</td>
                    <td>{{$shipping->updated_at}}</td>
                    <td>
                      <a href="{{route('shippings.edit', $shipping->id)}}" class="btn btn-info"><i class="fa fa-edit"></i> </a>
                      <a href="#" class="btn btn-danger" onclick="delete_this_item('{{$shipping->id}}')"><i class="fa fa-trash"></i> </a>
                    </td>
                  </tr>
              @endforeach
