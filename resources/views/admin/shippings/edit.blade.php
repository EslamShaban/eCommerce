...
@push('script')
 ...
 <?php 
      $lat = !empty($shipping->lat) ? $shipping->lat : '26.90147865970426';
      $lng = !empty($shipping->lng) ? $shipping->lng : '29.839355468749993';
 ?>
    ...
    <div class="card-body">
        ...
        <form action="{{route('shippings.update', $shipping->id)}}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group mb-3">
                ...
                <input type="text"  name="shippingname_ar" id="shippingname_ar" class="form-control" value="{{$shipping->shippingname_ar}}" placeholder="{{trans('admin.shippingname_ar')}}">
              </div>
              <div class="form-group mb-3">
                <label for="shippingname_en">{{trans('admin.shippingname_en')}}</label>
                <input type="text"  name="shippingname_en" id="shippingname_en" class="form-control" value="{{$shipping->shippingname_en}}" placeholder="{{trans('admin.shippingname_en')}}">
              </div> 
              <div class="form-group mb-3">
                ...
                <select name="user_id" id="user_id" class="form-control" value="{{old('user_id')}}">
                  ...
                  @foreach($users as $id=>$username)
                    <option {{$shipping->user_id == $id ? 'selected' : ''}} value="{{$id}}">{{$username}}</option>
                  @endForeach
                </select>
              </div>
              <div class="form-group mb-3">
              ...
              <div class="form-group mb-3">
                  ...
                  @if(! empty($shipping->logo))
                    <img src="{{Storage::url($shipping->logo)}}" class="imgPreview img-thumbnail" style="width:50px; height:50px">
                  @endif
              </div>
            <div class="col-3">
