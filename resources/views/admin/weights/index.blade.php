...
<div class="card">
    <div class="card-header">
      <h3 class="card-title">{{trans('admin.weights')}}</h3>
    </div>
    <!-- /.card-header -->
    <div class="card-body">
      <a href="{{route('weights.create')}}" class="btn btn-info"><i class="fa fa-plus"></i> <span>{{trans('admin.create_new_weight')}}</span></a>
      ...
      <form action="{{aurl('weights/destroy/all')}}" id="form_data" >
        ...
        <table id="example1" class="table table-bordered table-striped ">
          ...
          <tr>
            ...
            <th>#</th>
            <th>{{trans('admin.weightname_ar')}}</th>
            <th>{{trans('admin.weightname_en')}}</th>
          ...
          <tbody>
              ...
              @foreach($Weights as $weight)
                  ...
                  <tr style="height:10px">
                    <td><input type="checkbox" class="item_checkbox" name="item[]" onclick="item_check('{{$Weights->count()}}')" value="{{$weight->id}}" /></td>
                    <td>{{$i}}</td>
                    <td>{{$weight->weightname_ar}}</td>
                    <td>{{$weight->weightname_en}}</td>
                    <td>{{$weight->created_at}}</td>
                    <td>{{$weight->updated_at}}</td>
                    <td>
                      <a href="{{route('weights.edit', $weight->id)}}" class="btn btn-info"><i class="fa fa-edit"></i> </a>
                      <a href="#" class="btn btn-danger" onclick="delete_this_item('{{$weight->id}}')"><i class="fa fa-trash"></i> </a>
                    </td>
                  </tr>
              @endforeach
