...
<div class="card">
    <div class="card-header">
      <h3 class="card-title">{{trans('admin.colors')}}</h3>
    </div>
    <!-- /.card-header -->
    <div class="card-body">
      <a href="{{route('colors.create')}}" class="btn btn-info"><i class="fa fa-plus"></i> <span>{{trans('admin.create_new_color')}}</span></a>
      ...
      <form action="{{aurl('colors/destroy/all')}}" id="form_data" >
        ...
        <table id="example1" class="table table-bordered table-striped ">
          ...
          <tr>
            ...
            <th>#</th>
            <th>{{trans('admin.colorname_ar')}}</th>
            <th>{{trans('admin.colorname_en')}}</th>
          ...
          <tbody>
              ...
              @foreach($Colors as $color)
                  ...
                  <tr style="height:10px">
                    <td><input type="checkbox" class="item_checkbox" name="item[]" onclick="item_check('{{$Colors->count()}}')" value="{{$color->id}}" /></td>
                    <td>{{$i}}</td>
                    <td>{{$color->colorname_ar}}</td>
                    <td>{{$color->colorname_en}}</td>
                    <td><span style="width:30px; height:30px; display:inline-block; background-color:{{$color->color}}"></span></td>
                    <td>{{$color->created_at}}</td>
                    <td>{{$color->updated_at}}</td>
                    <td>
                      <a href="{{route('colors.edit', $color->id)}}" class="btn btn-info"><i class="fa fa-edit"></i> </a>
                      <a href="#" class="btn btn-danger" onclick="delete_this_item('{{$color->id}}')"><i class="fa fa-trash"></i> </a>
                    </td>
                  </tr>
              @endforeach
